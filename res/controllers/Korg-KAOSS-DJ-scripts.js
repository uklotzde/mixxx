...
KAOSSDJ.fxSwitch = function (channel, control, value, status, group) {
    var deck = KAOSSDJ.getDeckByChannel(channel);
    var fxUnit = '[EffectRack1_EffectUnit' + deck.deckNumber + ']';
    var fxCtrl = 'group_' + deck.group + '_enable';
    if (value === ON) {
        // Assign deck to fx unit
        engine.setValue(fxUnit, fxCtrl, 1);
        KAOSSDJ.updateDeckByChannel(channel, 'fx', true);
    } else {
        // Unassign deck from fx unit
        engine.setValue(fxUnit, fxCtrl, 0);
