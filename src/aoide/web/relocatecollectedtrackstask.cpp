...
mixxx::network::JsonWebRequest buildRequest(
    ...
    const auto oldUriPrefix = mixxx::EncodedUrl::fromQUrl(oldTrackDir.toQUrl());
    const auto newTrackDir = mixxx::FileInfo(QDir(newRootDir));
    const auto newUriPrefix = mixxx::EncodedUrl::fromQUrl(newTrackDir.toQUrl());
    kLogger.debug()
    ...
    const auto content = QJsonArray{QJsonObject{
            ...
            {QLatin1String("replacement"),
                    QJsonValue(newUriPrefix.toQString() + QLatin1Char('/'))},
    }};
    return buildRequest(
            collectionUid,
            content);
