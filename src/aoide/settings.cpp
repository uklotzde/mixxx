...
const ConfigKey kQueriesFilePathKey(kGroup, "queriesFilePath");
...
QString Settings::queriesFilePath() const {
    return m_userSettings->getValue(kQueriesFilePathKey);
}

void Settings::setQueriesFilePath(QString queriesFilePath) const {
    if (queriesFilePath.isEmpty()) {
        m_userSettings->remove(kQueriesFilePathKey);
    } else {
        m_userSettings->setValue(kQueriesFilePathKey, queriesFilePath);
    }
}
