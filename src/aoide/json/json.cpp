...
void Object::putOptional(QLatin1String key, std::optional<quint64> value) {
    if (value) {
        m_jsonObject.insert(key, static_cast<qint64>(*value));
    } else {
        m_jsonObject.remove(key);
    }
}
...
QStringList arrayToStringList(const QJsonArray& jsonArray) {
    ...
    for (auto&& jsonValue : std::as_const(jsonArray)) {
