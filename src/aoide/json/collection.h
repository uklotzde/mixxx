...
class ContentPathConfig : public Object {
  public:
    static ContentPathConfig forLocalFiles(
            const std::optional<mixxx::EncodedUrl>& rootUrl = std::nullopt);

    explicit ContentPathConfig(
            QJsonObject jsonObject = QJsonObject());
    ~ContentPathConfig() override = default;
    ...
    friend bool operator==(
            const ContentPathConfig& lhs,
            const ContentPathConfig& rhs) {
...
class MediaSourceConfig : public Object {
  public:
    ...
    ContentPathConfig contentPath() const;
    void setContentPath(
